      ...
      >
        {classes.map((c) => (
          <option key={c.id} value={c.id}>
            {c.name}
            {c.level ? ` — ${educationLevelLabel(c.level)}` : ""}
          </option>
        ))}
      </select>
    </div>
...
function AttendanceTab({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      onError(e instanceof Error ? e.message : "Impossible de charger l’appel.");
      setStudents([]);
    }
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-[var(--app-border)]">
            <tr>
              ...
              <th className="text-left px-4 py-2 font-medium">Présence</th>
            </tr>
          </thead>
          <tbody>
            {students.length === 0 ? (
...
function HomeworkTab({
  ...
  async function create(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Création refusée");
  ...
  async function saveGrade(studentId: string, score: string, comment: string) {
    ...
    const data = await res.json();
    if (!res.ok) {
      onError(data.message || "Note non enregistrée");
      return;
    }
    setDetail(data.assignment);
    onError("");
